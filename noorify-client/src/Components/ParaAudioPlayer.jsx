...
const ParaAudioPlayer = () => {
    ...
    if (suraList.length === 0) {
        return <p>Loading para list...</p>;
    }

    return (
        <div>
            <div>
                <p className="text-base font-medium text-white text-center py-3">Recreation by para/juz</p>
            </div>
            <div className="audio-player bg-white md:p-5 p-3 flex flex-col items-center justify-center rounded-xl">
                <h2 className="pb-3 text-center text-lg font-medium">{suraList[currentIndex].title}</h2>
                <audio
                    ref={audioRef}
                    controls
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg max-h-[80vh] w-full max-w-md overflow-y-auto">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-xl font-semibold">Select a Para</h3>
                                <button 
                                ...
                                {suraList.map((sura, index) => (
                                    <li
                                        ...
                                        className={`cursor-pointer p-2 rounded hover:bg-gray-100 ${
                                            index === currentIndex ? "font-bold bg-blue-50" : ""
                                        }`}
                                        onClick={() => {
...
export default ParaAudioPlayer;